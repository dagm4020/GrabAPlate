...
class _GroceryListState extends State<GroceryList> {
  ...
  List<String> categories = [];  List<bool> _expandedStates = [];  Map<int, List<Map<String, dynamic>>> foodItems = {};
  Map<String, int> categoryIds = {};  Timer? _timer;
...
void initState() {
  ...
  _loadCategoriesAndFoods();  _startPrintingDatabase();    _loadCategoriesAndFoods();
  ...
  Widget build(BuildContext context) {
    return Container(
      color: _darkModeEnabled ? Colors.black : Colors.white,      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,        children: [
          SizedBox(height: 10.0),
            ...
            padding: const EdgeInsets.symmetric(horizontal: 20.0),            child: Row(
              children: [
                Icon(
                  Icons.shopping_cart,                  color: _darkModeEnabled ? Colors.white : Colors.black,                ),
                ...
                Spacer(),                               GestureDetector(
                  ...
                  child: Container(
                    ...
                    padding: EdgeInsets.all(3.0),                    child: Icon(
                      Icons.add,
                      color: Colors.black,
                      size: 16,                    ),
                  ),
...
Future<void> _loadCategoriesAndFoods() async {
  final dbHelper = DatabaseHelper();
   ...
   setState(() {
    categories = categoriesData.map((category) => category['name'] as String).toList();
...
void _showPopupMenu(BuildContext context) async {
  ...
  await showMenu(
    ...
    items: [
      ...
      _buildPopupMenuItem('Nuts and Seeds', 'Almonds, walnuts, sunflower seeds.'),
      _buildPopupMenuItem('Fats and Oils', 'Olive oil, butter, avocado.'),
      _buildPopupMenuItem('Herbs and Spices', 'Fresh and dried herbs, spices for flavoring.'),
    ...
    if (selectedItem != null && selectedItem == 'Custom') {
      ...
      final dbHelper = DatabaseHelper();
           int categoryId = await dbHelper.addCategory(selectedItem, categories.length);
...
void _showCustomCategoryDialog(BuildContext context) {
  ...
  showDialog(
    ...
    builder: (BuildContext context) {
      return AlertDialog(
        backgroundColor: Colors.white,        title: Text('Enter Custom Category'),
        content: TextField(
          ...
          children: [
            ...
            AnimatedContainer(
              ...
              child: Transform.rotate(
                angle: _expandedStates.isNotEmpty && index < _expandedStates.length
                    ? (_expandedStates[index] ? 0 : 3.14)
                    : 0,                child: IconButton(
                  icon: Icon(Icons.arrow_forward),
                  onPressed: () {
                    setState(() {
                      if (_expandedStates.isNotEmpty && index < _expandedStates.length) {
            ...
            if (_expandedStates.isNotEmpty && index < _expandedStates.length && _expandedStates[index])
              Container(
                ...
                child: IconButton(
                  ...
                  onPressed: () {
                    if (categoryId != null) {
                      _showConfirmationDialog(categoryId, index);                    } else {
                                           ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
...
void _showAddFoodDialog(BuildContext context, int? categoryId) {
  ...
  showDialog(
    ...
    builder: (BuildContext context) {
      return AlertDialog(
        backgroundColor: Colors.white,        title: Text('Add Food to Category $categoryId'),
        content: Column(
          ...
          children: [
            ...
            DropdownButton<String>(
              ...
              dropdownColor: Colors.white,              onChanged: (String? newValue) {
                if (newValue != null) {
...
void _showMeasurementOptionsDialog(BuildContext context, String measurementType, TextEditingController foodController) {
  showDialog(
    context: context,
    builder: (BuildContext context) {
      return AlertDialog(
        backgroundColor: Colors.white,        title: Text('Select $measurementType Measurement'),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
...
Widget _buildMeasurementOption(BuildContext context, String measurement, TextEditingController foodController) {
  return ListTile(
    title: Text(measurement),
    onTap: () {
      Navigator.of(context).pop();      _showAddFoodDialogWithMeasurement(context, measurement, foodController);
    },
  );
}
...
void _showAddFoodDialogWithMeasurement(BuildContext context, String measurement, TextEditingController foodController) {
   showDialog(
    context: context,
    builder: (BuildContext context) {
      return AlertDialog(
        backgroundColor: Colors.white,        title: Text('Add Food'),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            TextField(
...
void _showVolumeOptionsDialog(BuildContext context, TextEditingController foodController) {
  showDialog(
    context: context,
    builder: (BuildContext context) {
      return AlertDialog(
        backgroundColor: Colors.white,        title: Text('Select Volume Measurement'),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            _buildMeasurementOption(context, 'Cup', foodController),
...
void _showWeightOptionsDialog(BuildContext context, TextEditingController foodController) {
  showDialog(
    context: context,
    builder: (BuildContext context) {
      return AlertDialog(
        backgroundColor: Colors.white,        title: Text('Select Weight Measurement'),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            _buildMeasurementOption(context, 'Lb', foodController),
...
void _showServingSizeOptionsDialog(BuildContext context, TextEditingController foodController) {
  showDialog(
    context: context,
    builder: (BuildContext context) {
      return AlertDialog(
        backgroundColor: Colors.white,        title: Text('Select Serving Size Measurement'),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            _buildMeasurementOption(context, 'Tbsp', foodController),
...
void _addNewCategory(String categoryName) async {
  final dbHelper = DatabaseHelper();
  int categoryId = await dbHelper.addCategory(categoryName, categories.length);
  setState(() {
