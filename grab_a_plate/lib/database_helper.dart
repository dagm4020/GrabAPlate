...
CREATE TABLE IF NOT EXISTS MealPlans (
  ...
  Future<List<Map<String, dynamic>>> getFavorites(int userId) async {
    final db = await database;
    return await db
        .query('Favorites', where: 'userId = ?', whereArgs: [userId]);
  }
  ...
  Future<List<Map<String, dynamic>>> getFavoriteMealsByUserId(
      int userId) async {
    final db = await database;
  ...
  Future<int> insertMealPlan(Map<String, dynamic> mealPlan) async {
    final db = await database;
    return await db.insert('MealPlans', mealPlan);
  }

  Future<Map<String, dynamic>?> getMealPlan(
      int userId, String date, String mealType) async {
    final db = await database;
    final List<Map<String, dynamic>> mealPlans = await db.query(
      'MealPlans',
      where: 'userId = ? AND date = ? AND mealType = ?',
  ...
  Future<int> updateMealPlan(int userId, String date, String mealType,
  ...
  Future<int> deleteMealPlan(int userId, String date, String mealType) async {
    final db = await database;
    return await db.delete(
      'MealPlans',
      where: 'userId = ? AND date = ? AND mealType = ?',
